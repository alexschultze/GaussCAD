classdef field_screen < handle & matlab.mixin.Copyable
    ...
    methods
        ...
        function [int,phase,e] = calc_int_phase_masked(self,par_mask)
            ...
            %Superposition of E1 and E2
            E_m = (self.E(:,:,1)+self.E(:,:,2)).*par_mask;
            
            % Local pixel intensity (irradiance)
            I =E_m.*conj(E_m); %per pixel
            int = mean(I,'all');

            % Local pixel contrast, for phase intensity contribution
        ...
        function ret_handle = plot_intensity(self)
            ...
            for i=1:size(self.E,3)
                subplot(3,1,1+i);
                ret_handle=surf(xx,yy,self.E(:,:,i).*conj(self.E(:,:,i)),'EdgeColor','none');hold on;
